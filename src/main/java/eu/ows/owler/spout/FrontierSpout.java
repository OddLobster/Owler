...
public class FrontierSpout extends AbstractQueryingSpout {

    private static final Logger LOG = LoggerFactory.getLogger(FrontierSpout.class);
    ...
    public void open(
        ...
        switch (addresses.size()) {
            ...
            case 1:
                ...
                break;
            default:
                LOG.debug("FrontierSpout Open called");
        ...
        LOG.debug("FrontierStub initialized");
    }
    ...
    protected void populateBuffer() {
        ...
        LOG.debug(
                "Populating buffer - max queues {} - max URLs per queues {} - delayRequestable {}",
                maxBucketNum * 10,
                maxURLsPerBucket * 10,
                delayRequestable);
        LOG.debug("Starting to populate buffer (hopefully)");
        GetParams request =
                GetParams.newBuilder()
                        .setMaxUrlsPerQueue(maxURLsPerBucket * 10)
                        .setMaxQueues(maxBucketNum * 10)
                ...
                new StreamObserver<>() {
                    ...
                    public void onCompleted() {
                        ...
                        LOG.debug(
                                ...
                                (end - start));
                        markQueryReceivedNow();
                        LOG.debug("OnCompleted finished");
                    }
                };

        LOG.trace("isInquery set to true");
        isInQuery.set(true);
        LOG.debug("gRPC call to GetURLs");
        frontier.getURLs(request, responseObserver);
        LOG.debug("End of populateBuffer");

        LOG.debug("Test start:");
        Local requestGetActive = Local.newBuilder().setLocal(true).build();

        StreamObserver<crawlercommons.urlfrontier.Urlfrontier.Boolean> responseObserverGetActive =
                new StreamObserver<crawlercommons.urlfrontier.Urlfrontier.Boolean>() {
                    @Override
                    public void onNext(crawlercommons.urlfrontier.Urlfrontier.Boolean value) {
                        boolean isActive = value.getState();
                        LOG.debug("onNext called | Active: " + isActive);
                    }
        ...
        frontier.getActive(requestGetActive, responseObserverGetActive);
        LOG.debug("Test end");
    }
    ...
    public void close() {
        ...
        if (!channel.isShutdown()) {
            LOG.debug("Shutting down connection to URLFrontier service.");
            channel.shutdown();
        } else {
            LOG.warn(
