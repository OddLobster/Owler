...
import eu.ows.owler.util.IdentifierGenerator;
import java.net.URI;
...
public class OWSWARCRecordFormat implements RecordFormat {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(OWSWARCRecordFormat.class);
    ...
    public OWSWARCRecordFormat(String protocolMDprefix, List<String> customMetadata) {
    ...
    public byte[] format(Tuple tuple) {
        ...
        // OWS: Add custom fields
        String urlId;
        try {
            urlId = IdentifierGenerator.generateUrlId(url);
        } catch (URISyntaxException e) {
            LOG.error("Exception caught when generating URL ID for {}", url, e);
            urlId = "ERROR";
        }
        buffer.append("OWS.URL-ID").append(": ").append(urlId).append(CRLF);

        for (final String key : customMetadata) {
            final String[] values = metadata.getValues(key);
            if (values != null) {
                final StringBuilder concatenatedValues = new StringBuilder();
                for (int i = 0; i < values.length; i++) {
                    concatenatedValues.append(values[i]);
                    if (i < values.length - 1) {
                        concatenatedValues.append(", ");
                    }
                }
                buffer.append("OWS." + key.toUpperCase(Locale.ROOT))
                        .append(": ")
                        .append(concatenatedValues.toString().toUpperCase(Locale.ROOT))
                        .append(CRLF);
            }
        }
